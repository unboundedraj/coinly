...
export function TransactionDialog({ trigger: _trigger, triggerLabel = "Add Transaction", hideTrigger = false, initialValues, open: controlledOpen, onOpenChange }: { trigger?: React.ReactNode; triggerLabel?: string; hideTrigger?: boolean; initialValues?: Partial<TransactionInput>; open?: boolean; onOpenChange?: (open: boolean) => void }) {
  const [accounts, setAccounts] = useState<AccountOption[]>([]);
  const [uncontrolledOpen, setUncontrolledOpen] = useState(false);
  const open = controlledOpen ?? uncontrolledOpen;
  const [submitError, setSubmitError] = useState("");
  const [successMessage, setSuccessMessage] = useState("");
  const form = useForm<TransactionInput>({ resolver: zodResolver(transactionSchema), defaultValues: { type: "EXPENSE", amount: 0, category: "Food", date: new Date().toISOString().slice(0, 10), isRecurring: false, ...initialValues } });
  const type = form.watch("type");

  useEffect(() => { if (open) void getAccounts().then(setAccounts).catch(() => setAccounts([])); }, [open]);
  useEffect(() => { if (initialValues) form.reset({ type: "EXPENSE", amount: 0, category: "Food", date: new Date().toISOString().slice(0, 10), isRecurring: false, ...initialValues }); }, [initialValues]);
  ...
  async function submit(values: TransactionInput) {
    setSubmitError("");
    setSuccessMessage("");
    try { await createTransaction(values); form.reset({ type: "EXPENSE", amount: 0, category: "Food", date: new Date().toISOString().slice(0, 10), isRecurring: false }); setDialogOpen(false); setSuccessMessage("Transaction added successfully"); } catch (error) { setSubmitError(error instanceof Error ? error.message : "Unable to save transaction."); }
  }
